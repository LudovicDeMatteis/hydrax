...
def run_interactive(  # noqa: PLR0912, PLR0915
    ...
    print(f"Loading MPC data from: {mpc_log_path}")
    if not os.path.exists(mpc_log_path):
        raise FileNotFoundError(f"Could not find MPC file at {mpc_log_path}")
    mpc_data = np.load(mpc_log_path)
    mpc_ctrl = mpc_data["CEM_mean"]
    
    print(f"Loading state data from: {state_log_path}")
    if not os.path.exists(state_log_path):
        raise FileNotFoundError(f"Could not find state file at {state_log_path}")
    state_data = np.load(state_log_path)
    state_qpos = state_data["qpos"]
    state_qvel = state_data["qvel"]
    state_qacc = state_data["qacc"]
    state_ctrl = state_data["ctrl"]
    ...
    with viewer_context as viewer:
        ...
        while True:
            ...
            new_tk = (
                jnp.linspace(0.0, controller.plan_horizon, controller.num_knots) + mj_data.time
            )
            new_mean = mpc_ctrl[k]
            policy_params = policy_params.replace(tk=new_tk, mean=new_mean)
            k += 1
            ...
            for i in range(sim_steps_per_replan):
                ...
                mj_data.ctrl[:] = state_ctrl[(k-1)*sim_steps_per_replan+i]
                ...
                if reference is not None:
                    ...
                    for j, site_id in enumerate(site_ids):
                        mj_data.site_xpos[site_id] = ref_positions[j]
                ...
                print("---")     
                print("qpos RMSE", np.linalg.norm(mj_data.qpos - state_qpos[(k-1)*sim_steps_per_replan+i]))
                print("qvel RMSE", np.linalg.norm(mj_data.qvel - state_qvel[(k-1)*sim_steps_per_replan+i]))
                print("qacc RMSE", np.linalg.norm(mj_data.qacc - state_qacc[(k-1)*sim_steps_per_replan+i]))
